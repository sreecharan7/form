window.alert("this may change the data that are already submitted ,please note that!!")
    ...
    fetch("/form/edit/data", {
      ...
      .then(data => {
        if(data.error){
            ...
            linkshower("http://localhost:5000\\form\\"+key.key);
        }
      })
   }
   else{
...
function linkshower(link){
    ...
    $("#staticBackdropLabel").html("updated sucefully")
...
function reloadThedata(data){
    $("#nameOfForm").val(data.name);
    $("#About-form").val(data.description);
    for(let i=0;i<data["q"].length;i++){
        if(i!=0){addQuestionBox();}
        $("#"+(i+1)+"-question #question").val(data["q"][i]["question"]);
        var n=data["q"][i]["options"].length;
        if(n==1){optionremove(i+1);}
        var containerOptions=optionadd(i+1,n-2);
        var opt=containerOptions.querySelectorAll(".options");
        // console.log(opt);
        for(let j=0;j<n;j++){
            $(opt[j]).val(data["q"][i]["options"][j]);
        }
        if(data["q"][i]["type"]=="multiple"){
            $("#"+(i+1)+"-question #checkbox").prop("checked", true);
        }
    }
}


//fencthing the data from the server
...
fetch("/form/data/question", {
  ...
  .then(data => {
    ...
    reloadThedata(data.dataQuestion);
  })
